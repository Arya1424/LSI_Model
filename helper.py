...
from modules.kdtree_retrieval import KDTreeRetriever # NEW: Import KD-Tree
    ...
    if not train_data.sent_vectorized:
        ...
        for instance in tqdm(train_data.dataset + label_data.dataset, desc="Creating vocabulary"):
            if isinstance(instance['text'], np.ndarray) and instance['text'].ndim == 1:
                ...
                # Fallback for original untokenized text
...
def generate_label_weights(train_data, label_vocab, dev='cuda:0', scheme="tws", thresh=10.):
    ...
    for instance in tqdm(train_data, desc="Generating label weights"):
        if 'labels' in instance and instance['labels'] is not None:
             for l in instance['labels']:
                if l in label_vocab: # Guard against labels not in vocab
                    pos[label_vocab[l]] += 1
    ...
    if scheme == 'tws':
        weights = torch.clamp(pos.max() / pos_safe, max=thresh)
    else: # vws
        weights = len(train_data) / pos_safe
...
def train_dev_pass(model, optimizer, fact_loader, sec_batch, metrics=None, pred_threshold=None, train=False, infer=False, label_vocab=False, kdtree: KDTreeRetriever = None): 
    ...
    for i, fact_batch in enumerate(tqdm(fact_loader, desc="Flowing data through model")):    
        ...
        # Model now returns 4 values: loss, predictions, sec_struct_hidden, fact_attr_hidden
        loss, predictions, sec_struct_hidden, fact_attr_hidden = model(fact_batch.cuda(), sec_batch.cuda(), pthresh=pred_threshold)
        ...
        if not infer:
            ...
            if kdtree is not None:
                # 1. Query KD-Tree using fact_attr_hidden (fact attribute embedding)
                ...
                # 2. Filter predictions to only include top-K candidates
                ...
                for doc_idx in range(predictions.size(0)):
                    ...
                    # Apply mask: predictions must be 1 AND in the candidate set
...
class MultiLabelMetrics(torch.nn.Module):
    ...
    def forward(self, predictions, labels, loss=None):
        ...
        jacc_score = intersection / (union + 1e-10)
        
        self.run_jacc += jacc_score.sum().item()
    ...
    def calculate_metrics(self, refresh=True):
        ...
        self.micro_rec = match_total / labels_total if labels_total > 0 else 0
        self.micro_f1 = 0 if self.micro_prec + self.micro_rec == 0 else 2 * self.micro_prec * self.micro_rec / (self.micro_prec + self.micro_rec) 
        
        self.jacc = self.run_jacc / self.counter if self.counter > 0 else 0
        if 'run_loss' in self.__dict__:
            self.loss = self.run_loss / self.counter if self.counter > 0 else 0
