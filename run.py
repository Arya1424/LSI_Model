...
if dc.get('s2v_path') is not None:
    print(f"Attempting to load Sent2Vec model from {dc['s2v_path']}")
    try:
        sent2vec_model = sent2vec.Sent2vecModel()
        sent2vec_model.load_model(dc['s2v_path'])
    except Exception as e:
        print(f"Warning: Could not load sent2vec model. Error: {e}")
        sent2vec_model = None
else:
    sent2vec_model = None
...
print("\nPreparing Datasets")
print("==================================================")
# NOTE: Pass new flags to preprocess
sec_dataset = LSIDataset(jsonl_file=dc['sec_src'])
sec_dataset.preprocess(use_trie=use_trie, use_orwell=use_orwell)
...
if hc['do_train_dev']:
    ...
    train_dataset.preprocess(use_trie=use_trie, use_orwell=use_orwell)
    ...
    dev_dataset.preprocess(use_trie=use_trie, use_orwell=use_orwell)
...
if hc['do_test']:
    ...
    test_dataset.preprocess(use_trie=use_trie, use_orwell=use_orwell)
...
if hc['do_infer']:
    ...
    infer_dataset.preprocess(use_trie=use_trie, use_orwell=use_orwell)
...
sec_weights = generate_label_weights(train_dataset, label_vocab, scheme=hc['weight_scheme'], thresh=hc['tws_thresh'])
...
print("\nPreparing Model")
print("==================================================")
# Pass orwell_dim to the model constructor
lsc_model = LeSICiN(
    hc['hidden_size'], 
    L, 
    N, 
...
# Initialize KD-Tree outside of the training loop
kdtree = None
if use_kdtree:
...
if hc['do_train_dev']:
    ...
    for epoch in range(hc['num_epochs']):
        # train_dev_pass returns 4 values, but we only need metrics here
    ...
    # NEW: Build KD-Tree from the best model's structural embeddings
    if use_kdtree:
...
if hc['do_test']:
    ...
    print("\nRunning Test")
    print("==================================================")
    # The default test evaluation does NOT use KD-Tree for fair comparison (full score matrix is evaluated)
...
if hc['do_infer']:
    ...
    # Infer outputs are computed with KD-Tree if available
    infer_outputs = train_dev_pass(
        ...
        label_vocab=label_vocab,
        kdtree=kdtree # PASS KD-Tree HERE
    )
