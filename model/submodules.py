...
class HierAttnNet(torch.nn.Module):
    ...
    def forward(self, tokens=None, doc_inputs=None, mask=None, sent_dyn_context=None, doc_dyn_context=None): 
        # tokens: [B, S, W], doc_inputs: [B, S, H], mask: [B, S, W] / [B, S]
...
class MetapathAggrNet(torch.nn.Module):
    ...
    def forward(self, tokens, edge_tokens, schemas, intra_context=None, inter_context=None): 
        ...
        # Use dynamic context (from text attribute encoder) if provided, otherwise use static context
...
class MatchNet(torch.nn.Module):
    # Implements the scoring function m_theta (Fact, {Sections})
    def __init__(self, hidden_size, num_labels, drop=0.1):
        super().__init__()
        
        self.match_lstm = LstmNet(hidden_size) # Section Contextualization (h_s -> ~h_s)
        self.match_attn = AttnNet(hidden_size, drop=drop) # Section Attention (~h_s -> h_S)
        self.match_fc = torch.nn.Linear(2 * hidden_size, num_labels) # Final Classifier (Fact, h_S -> Logits)
